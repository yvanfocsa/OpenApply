...
6. Tailor the title, summary, skills, experience wording, and project emphasis using verified facts. Contract mentions (e.g. "| CANDIDATURE CDI", "| CANDIDATURE ALTERNANCE", or "| APPRENTICESHIP") are permitted in the CV title/header (P[2]), but the job title must be kept concise and trimmed (e.g. "CYBERSECURITY APPRENTICE | APPRENTICESHIP") so the total line length never overflows or collides with the candidate photo on the top right. Preserve employers, dates, education, contact details, visual hierarchy, styles, margins, columns, tables, media, and section order.
...
Templates can come from any profession and may use paragraphs or tables. Do not assume fixed paragraph indexes. Identify sections from their visible headings and surrounding structure. Prefer minimal in-place text edits that retain existing run and paragraph properties. When a field cannot be safely identified, preserve it instead of guessing. Contract tags are allowed in the CV title line (P[2]), provided the overall title remains concise to prevent text overflow under the photo. Never reassign `paragraph.text = ...` on existing paragraphs, as this destroys run formatting (bold, italics), tab stops, and embedded media (such as the candidate photo anchored in P[0]). Always update specific `run.text` elements in-place.

STRICT LANGUAGE PURITY: Ensure 100% language consistency across the entire document set. When the target language is English, do not allow any French words in the headers, title, contract mention, subject line, greeting, dates, or body text (e.g., use "APPRENTICESHIP" instead of "alternance", "PERMANENT POSITION" instead of "CDI", "Subject: Application for..." instead of "Objet :", "Dear Hiring Manager," instead of "Madame, Monsieur,", "Sincerely," instead of "Salutations distinguées"). When French, keep all text strictly in French. Never mix languages.
